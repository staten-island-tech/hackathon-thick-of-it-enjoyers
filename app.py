...
def home_screen():
    ...
    while running:
        screen.fill(BLUE)
        # Draw title
        title_text = font.render("Falling Blocks Game", True, WHITE)
        ...
        speed_text = font.render(f"Speed: {falling_speed}", True, WHITE)
        screen.blit(speed_text, (screen_width // 2 - speed_text.get_width() // 2, 200))

        # Character Customization
        customization_text = font.render("Press C to Customize Character", True, WHITE)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_s:  # Change speed
                    falling_speed = (falling_speed % 10) + 1  # Cycle speed from 1 to 10
                if event.key == pygame.K_c:  # Customize character
...
def game_loop():
    ...
    while running:
        ...
        if keys[pygame.K_w] and char_y > 0:
            char_y -= char_speed
        if keys[pygame.K_s] and char_y < screen_height - char_height:
            char_y += char_speed
...
def home_screen():
    ...
    while running:
        ...
        if prompt_active:
            input_prompt_text = font.render("Enter Speed (1-10):", True, WHITE)
            ...
            input_display_text = font.render(input_text, True, WHITE)
